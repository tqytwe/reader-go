...
- 📚 **本地书籍**：支持 TXT / EPUB / CBZ 格式解析，拖拽上传
- 🛠 **书源管理**：增删改查、导入导出、批量删除、批量启用/禁用
- 📋 **书架管理**：分组、排序、阅读进度追踪、批量移除
- 🔄 **替换规则**：正则替换，净化阅读内容，支持批量操作
- 📡 **RSS 订阅**：订阅源管理、批量删除、分组筛选
- 🏠 **首页仪表盘**：统计概览、继续阅读、快捷入口
- 🔒 **密码锁屏**：前端密码保护，SHA-256 加密存储
- 🎨 **主题切换**：支持亮色/暗色/护眼模式
- 📱 **响应式设计**：可折叠侧边栏，移动端适配
